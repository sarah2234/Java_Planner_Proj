...
public class ServerBackground {
    ...
    public void setting() throws IOException {
        ...
        while (true) {
            ...
            Receiver receiver = new Receiver(socket);   //새로운 사용자 쓰레드 생성 후 소켓정보 넣기
            receiver.start();
        }
    }
        ...
        public void run() {     //사용자로부터 받을 내용
            try {// 계속 듣기만!!
                while (in != null) {
                    ...
                    if("all".equals(token[0])) {        //모두에게 보냄
                        sendMessage(token[1]);   //사용자로부터 받은 내용을 사용자에게 뿌림, 단 문자열에 : 포함되면 안됨
                        gui.appendMsg(msg); //사용자로부터 받은 내용을 서버 gui에 뿌림
                    }
                    else if("one".equals(token[0])) {   //한명에게 보냄
                        sendOne(nick, token[1]);
                        gui.appendMsg(msg);
                    }
                }
            } catch (IOException e) {
