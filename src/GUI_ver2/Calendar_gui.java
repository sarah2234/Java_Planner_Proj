...
public class Calendar_gui extends JPanel implements ItemListener, ActionListener{
    ...
    public void setDay() {
        ...
        for(int day=1; day<=lastDay; day++) {
            JButton btn = new JButton(String.valueOf(day)); //버튼선언해주는데 String.value 는 형변환이다. JButton을 가운데에 입력하게둔다.
            btn.setFont(fnt); //라벨에 폰트를 주입한다.
            //출력하는 날짜에 대한 요일
    ...
    public void itemStateChanged(ItemEvent e) { //콤보박스를 변경하였을때에 선택되는 이벤트이다.
        ...
        setDay(); //날짜 처리 함수 호출
        dayPane.setVisible(true); //패널을 볼수있게 처리한다.     다시 한 번 확인해보자
